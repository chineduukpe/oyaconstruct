...
@section('content')
    ...
    <div class="row m-2 mt-5">
        ...
        <div class="col-md-4">
            <div class="row">
                <div class="col-md-12 justify-content-center img-magnifier-container" style="max-height: 40vh">
                    <img src="{{route('image.get',$products->productpic)}}" alt="{{$products->productname}} "
                         id="productMainPhoto"
                            ...
                            @foreach($products->pictures as $picture)
                                <div class="col-sm-2 m-1" style="height: 50px; ">
                                    {{--<div class="card-body ml-1" style="height: 200px; width: 200px;">--}}
                                        <img src="{{route('image.get',$picture->picturename)}}"
                                             alt="{{$products->productname}}" style="max-height: 100%;" class="img img-responsive  product-alt-photo">
                                    {{--</div>--}}
                                </div>
                            @endforeach
                        @endif
        ...
        <div class="col-md-6 ">
            ...
            <div class="row mt-4 mb-1">
                <div class="col-md-12">
                    <h2><del>N</del>
                        {{$products->price ? $products->price : $products->productPrices()->get()->first()->price   }}
                    </h2>
                </div>
                <div class="col-sm-12">
                ...
                <div class="col-sm-12 mt-4">
                    ...
                    @foreach($products->colours as $colour)
                        ...
                        <input type="radio" class="product-purchase-colour" value="{{$colour->id}}" name='product-clour' style="background: {{$colour->colour}}; display: inline-block; cursor:pointer " >
                    </div>
                    @endforeach
                    @endif
                </div>
                <div class="col-sm-12 mt-4">
                    ...
                    <div class="row">
                    @if($products->productPrices()->get())
                    @foreach($products->productPrices()->get() as $price)
                        <div class="col-sm-2 col-md-1 ml-2 " style="display: inline-block; "> <input type="radio" name="size" class='bg-warning' value="{{$price->first()->size()->first()->id}}"> {{$price->first()->size()->first()->size}}</div>
                    @endforeach
                    @endif
                    </div>
                </div>
            ...
            <div class="row m-3">
                ...
                <a href="#" class="btn btn-lg btn-warning m-1"> <i class="fa fa-cart-plus"></i> Add to Cart</a>
            </div>
            </form>
        </div>
        {{--END PRODUCT DETAILS--}}
    ...
    <div class="row">
        ...
        <p class="mb-4 pl-3">Store Related Products</p>
            <div class="row pl-3 ">
                ...
                @foreach($products->store()->first()->products()->paginate(5) as $product)
                    @if(!($products->id == $product->id))
                        <div class="col-sm-2">
                            <div class="alt-image-container ">
                                <div class="card-head justify-content-center ">
                                    <a href="{{route('product.view',$product->id)}}">
                                        <img src="{{route('image.get',$product->productpic)}}" alt="" class="img img-responsive" style="height: 100px;">
                                    </a>
                                </div>
    ...
    <div class="row mt-5">
        <div class="col-md-12 border-bottom p-5" >
            <h1>Check back soon for awesome promotions!</h1>
        </div>
    </div>
    {{--START REVIEEW--}}
    <div class="row">
...
@section('script')
    ...
    <script>
        {{--Make Sure orders must me one and above--}}
