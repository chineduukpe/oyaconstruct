...
class UserController extends Controller
{
	...
	{
		// Check if the user entered their username and password
		if (count(explode(' ', $request->name)) < 2) {
			return redirect()->back()->with('error', 'Seems you forgot to enter your first name or your last name');
		}
		try {
			DB::beginTransaction();
			$request->validate([
	...
	public function cancelOrder($order_id)
	{
		...
		if(!$order){
			return redirect(route('customer.orders.view'))->with('error','Seems the item does not exist or has been moved permanently');
		}
		...
		try{
			$order->delete();
			return redirect(route('customer.orders.view'))->with('success','Your order has been canceled. ');
		}
		catch(Exception $e){
			return redirect(route('customer.orders.view'))->with('error','Seems like a network error has occured. Try again.');
		}
	}
}
