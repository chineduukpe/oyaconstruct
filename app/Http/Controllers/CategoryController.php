...
public function allcatjson(Request $request){
        $cats=category::where('servicetype',$request->service)->orderBy('catname','asc')->get();
        $i=0;
        $data='[{"id":"","catname":"","servicetype":"","subcat":""}';
        foreach ($cats as $cat) {
            # code...
            $subcats=subcategory::where('catname',$cat->id)->get();
            $subdata='[{"subid":"","subcat":""}';
            $j=0;
            foreach ($subcats as $subcat) {
                # code...
                $subdata.=',{"subid":"'.$subcat->id.'","subcat":"'.$subcat->subcat.'"}';
                $j++;
            }
            $subdata.=']';
            $data.=',{"id":"'.$cat->id .'", "catname":"'.$cat->catname.'", "servicetype":"'.$cat->servicetype.'","subcat":'.$subdata.'}';
            $i++;
        }
        $data.=']';
        echo $data;
